...
echo '<div class=" container-fluid" style="margin-top: 5%;">
	<div class="col-md-8 offset-md-2">
		<div class="card bg-light mb-3" style="min-width: 100%;"><div class="card-header">CREATE USER ACCOUNT</div>
		  	<div class="card-body">
		  		<div class="container">
				  <div class="row">
				    <div class="col-sm">';
...
echo button( 'add', '', '', 'smaller' ) . '&nbsp;' . _( 'Add Profile Picture' );
?></a><br />
...
if ( AllowEdit() && ! isset( $_REQUEST['_ROSARIO_PDF'] ) )
{
	...
	$user_name_html = '<div class="form-group">' .
	SelectInput(issetVal( $staff['TITLE'], '' ), 'staff[TITLE]','', $titles_array, '','class="form-control"> placeholder="Select Title"', $div) . '</div><div class="form-group">' .
	TextInput(issetVal( $staff['FIRST_NAME'], '' ),'staff[FIRST_NAME]', '','maxlength=50 class="form-control" placeholder="First name (required)" required',$div) . '</div><div class="form-group">' .
	TextInput(issetVal( $staff['MIDDLE_NAME'], '' ), 'staff[MIDDLE_NAME]', '', 'maxlength=50 placeholder="Middle name" class="form-control"', $div) . '</div><div class="form-group">' .
	TextInput( issetVal( $staff['LAST_NAME'], '' ), 'staff[LAST_NAME]', '', 'maxlength=50 class="form-control" placeholder="Last name (required)" required', $div) . '</div><div class="form-group">' .
	SelectInput(issetVal( $staff['NAME_SUFFIX'], '' ), 'staff[NAME_SUFFIX]', '', $suffixes_array,'', 'class="form-control"> placeholder="Name Suffix"',$div) . '</div>';
...
echo TextInput(
	issetVal( $staff['USERNAME'], '' ), 'staff[USERNAME]', '', 'class="form-control" placeholder="Username (required)" size=12 maxlength=100 autocomplete="off" ' . ( $required ? 'required' : '' ),
	empty( $_REQUEST['moodle_create_user'] )
);
...
echo PasswordInput(
	...
	'staff[PASSWORD]','' . ( ! empty( $_REQUEST['moodle_create_user'] ) ? '<div class="tooltip"><i>' .
		...
		'</i></div>' : ''), 'maxlength="42" tabindex="2" class="form-control" style="min-width: 300px;" placeholder="Password (required)" strength' .
...
echo '</div></div><tr class="st"><td colspan="2">';
